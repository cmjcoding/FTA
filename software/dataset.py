...
class Dataset:
    def __init__(self, dataset,datadir='./data'):
    ...
    def get_entity_types(self):
        """新增方法：加载实体类型信息"""
        type_path = f'./data/{self.dataset}/entity2type.txt'
        if not os.path.exists(type_path):
            raise FileNotFoundError(f"需先准备实体类型文件: {type_path}")

        entity_types = {}
        with open(type_path) as f:
            for line in f:
                eid, types = line.strip().split('\t')
                entity_types[int(eid)] = types.split('|')  # 支持多类型
        return entity_types
